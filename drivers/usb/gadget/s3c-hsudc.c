...
#define ep_index(_ep)		((_ep)->bEndpointAddress & USB_ENDPOINT_NUMBER_MASK)

static const char driver_name[] = "s3c-udc";
static const char ep0name[] = "ep0-control";
static struct s3c_hsudc *s3c_hsudc_dev;
...
static inline void __orr32(void *ptr, u32 val)
{
...
static void s3c_hsudc_init_phy(void)
{
	...
	cfg = readl(PWRCFG) | (1<<4);
	writel(cfg, PWRCFG);

	cfg = readl(USB_RSTCON);
	cfg |= (1<<2) | (1<<0);
	writel(cfg, USB_RSTCON);
	mdelay(1);
	...
	writel((1<<2) | (1<<1), USB_PHYCTRL);

	cfg = (3<<4);
	writel(cfg, USB_PHYPWR);

	cfg = readl(USB_CLKCON);
	cfg |= (1<<31) | (1<<2);
	writel(cfg, USB_CLKCON);
}
...
static void s3c_hsudc_uninit_phy(void)
{
	...
	writel((3<<4) | (1<<0), USB_PHYPWR);

	cfg = readl(USB_CLKCON) & ~((1<<31) | (1<<2));
	writel(cfg, USB_CLKCON);
}
...
{
	struct s3c_hsudc_ep *hsep;
	int set = (ctrl->bRequest == USB_REQ_SET_FEATURE);
 ...
 */
static int s3c_hsudc_irq(int irq, void *_dev)
{
...
int s3c_hsudc_probe(struct s3c24xx_hsudc_platdata *pdata)
{
	struct s3c_hsudc *hsudc;
	struct s3c24xx_hsudc_platdata *pd = pdata;
	int ret;
	...
	s3c_hsudc_dev = hsudc;
	hsudc->pd = pdata;
	hsudc->regs = pdata->regs;

	hsudc->gadget.ops = &s3c_hsudc_gadget_ops;
	hsudc->gadget.name = "s3c-hsudc";
...
{
	struct s3c_hsudc *hsudc = s3c_hsudc_dev;
	int ret;
...
int usb_gadget_handle_interrupts(void)
{
	...
	u32 ep_intr = readl(hsudc->regs + S3C_EIR) & 0x3FF;

	if (ep_intr)
		return s3c_hsudc_irq(0, hsudc);
	return 0;
}
