...
static inline struct s3c2416_mmc *s3c2416_get_base_mmc(int dev_index)
{
	unsigned long offset = dev_index * 0x400000;
	return (struct s3c2416_mmc *)(0x4ac00000 - offset);
}
...
static void mmc_change_clock(struct mmc_host *host, uint clock)
{
	...
	/* Wait max 10 ms */
	timeout = 10;
	while (!(readw(&host->reg->clkcon) & (1 << 3))) {
		if (timeout == 0) {
			printf("%s: timeout error\n", __func__);
			return;
		}
...
static int s3c2416_mmc_initialize(int dev_index, int bus_width)
{
	struct mmc *mmc;
	...
	mmc_host[dev_index].reg = s3c2416_get_base_mmc((dev_index==2) ? 1 : dev_index);
	mmc->b_max = 0;
	mmc_register(mmc);
...
int s3c2416_mmc_init(int dev_index, int bus_width)
{
	return s3c2416_mmc_initialize(dev_index, bus_width);
}
