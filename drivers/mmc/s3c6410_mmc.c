...
static inline struct s3c6410_mmc *s3c6410_get_base_mmc(int dev_index)
{
	unsigned long offset = dev_index * sizeof(struct s3c6410_mmc);
	return (struct s3c6410_mmc *)(0x7c200000 + offset);
}
...
static int s3c6410_mmc_initialize(int dev_index, int bus_width)
{
	struct mmc *mmc;
	...
	mmc_host[dev_index].reg = s3c6410_get_base_mmc(dev_index);
	mmc->b_max = 0;
	mmc_register(mmc);
...
int s3c6410_mmc_init(int dev_index, int bus_width)
{
	return s3c6410_mmc_initialize(dev_index, bus_width);
}
